...
const Enemy: React.FC<EnemyProps> = ({ enemyState }) => {
  const { x, y, type, bossType, hp, attackState, statIncreaseText } = enemyState;
  ...
  if (showAttackAnimation) {
    if (type === 'knight' || type === 'knight_shooter' || type.includes('angel')) {
  ...
  if (type === 'boss') {
    ...
    const maxHp = bossInfo.maxHp * Math.pow(4, Math.floor(hp / bossInfo.maxHp / 2)); // A bit of a guess, need better state
    const healthPercentage = (hp / maxHp) * 100;

    return (
        <div
            className="absolute w-[50px] h-[50px] border-2 border-black z-0 transition-transform duration-200 hover:scale-105"
            style={{ left: `${x}px`, top: `${y}px`, backgroundColor: bossInfo.color }}
        >
